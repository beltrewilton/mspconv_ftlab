...
def load_audio_data(df_annotations: pd.DataFrame, pc_num: int = None, audio_name: str = None) -> (np.ndarray, int):
    """
        Inputs:
...
def load_audio_part_data(df_annotations: pd.DataFrame, part_num: int, pc_num: int = None, audio_name: str = None) -> (np.ndarray, int):
    """
        Inputs:
            -df_annotations: Dataset annotations directory. For every file contains contains a row with the name, emotion, annotator, podcast part and number.
            -part_num: Audio Part
